# Putting this in actual Arabic screws up padding and alignment.
language-name = Arabic

# AUR Packages (-A)
A-install-deps = .تحديد التبعيات
A-install-repo-pkgs = :تبعيات المستودع
A-install-aur-pkgs = :AURرزمةاﻟ
# A-install-path-comp = Failed to extract final component of: { $path }
A-install-ignored = تم تحديده كمجهول. هل تريد ان تحمل على أي حال؟ { $file }
...
A-build-hotedit-install = ؟.installهل تريد تعديل ملف اﻟ
A-build-fail = .لقد فشل البناء
# A-build-e-pkgctl = Building within an isolated chroot failed.
A-build-e-makepkg = .makepkgهناك فشل في اﻟ
...
A-u-fetch-info = ...احضار معلومات الحزمة
A-u-comparing = ...مقارنة اصدارات الحزمة
A-u-no-upgrades = .AURلا يلزم تحديث حزمة اﻟ
A-u-to-upgrade = للتحديث AURرزمة اﻟ
...
C-b-target = { $target } التنسيخ الاحتياطي الى
...
C-downgrade-which = الذي تريده؟. { $pkg } ما هو اصدار
...
env-pconf = .الخاص بك pacman.confفشل تحليل ملف اﻟ
