...
# AUR Packages (-A)
A-install-deps = Određivanje zavisnosti...
A-install-repo-pkgs = Zavisnosti iz repozitorija:
A-install-aur-pkgs = AUR Paketi:
...
A-build-fail = Izgradnja nije uspjela.
...
A-build-continue = Želite li svejedno nastaviti?

A-i-repo = Repozitorij
A-i-version = Verzija
A-i-status = AUR Stanje
# A-i-maintainer = Maintainer
A-i-proj-url = URL Projekta
# A-i-aur-url = AUR URL
A-i-license = Licenca
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Zavisnosti
# A-i-make = Make Deps
...
A-u-fetch-info = Preuzimanje podataka o paketima...
A-u-comparing = Uspoređivanje verzija paketa...
A-u-no-upgrades = Svi AUR paketi su ažurirani.
A-u-to-upgrade = AUR paketi za nadogradnju:
...
# Snapshots (-B)
B-saved = Stanje paketa spremljeno.
...
C-b-target = Stvaram sigurnosnu kopiju u { $target }
...
C-c-keep = { $pkgs } zadnjih verzija svakog paketa će biti zadržano. Ostali paketi će biti izbrisani.
# C-c-freed = { $bytes } freed.

C-downgrade-which = Koju verziju paketa { $pkg } želite?
...
# Logs (-L)
L-first = Prva instalacija
L-upgrades = Nadogradnje
L-recent = Nedavne radnje
