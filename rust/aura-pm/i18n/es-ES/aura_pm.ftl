...
# AUR Packages (-A)
A-install-deps = Determinando dependencias...
A-install-repo-pkgs = Dependencias del repositorio:
A-install-aur-pkgs = AUR Paquetes:
# A-install-path-comp = Failed to extract final component of: { $path }
A-install-ignored = { $file } está marcado como ignorado. ¿Deseas instalarlo de todas formas?

# A-build-prep = Preparing build directories...
A-build-pkg = Compilando { $pkg }...
# A-build-diff = Display diffs of build files?
# A-build-hotedit-pkgbuild = Edit the PKGBUILD?
A-build-hotedit-install = ¿Desea editar el archivo .install?
A-build-fail = La compilación falló.
# A-build-e-pkgctl = Building within an isolated chroot failed.
A-build-e-makepkg = Ocurrió un error al ejecutar makepkg.
...
A-build-pull = Error al 'git pull' las últimas actualizaciones.
A-build-continue = ¿Deseas continuar de todas formas?

A-i-repo = Repositorio
A-i-version = Versión
A-i-status = Estado en AUR
A-i-maintainer = Mantenedor
A-i-proj-url = URL del proyecto
A-i-aur-url = URL de AUR
A-i-license = Licencia
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Dependencias
A-i-make = Dependencias de compilación
# A-i-opt = Optional Deps
# A-i-check = Check Deps
A-i-votes = Votos
A-i-pop = Popularidad
A-i-desc = Descripción
# A-i-keywords = Keywords
# A-i-submitted = Submitted
# A-i-updated = Updated

A-u-fetch-info = Obteniendo información de los paquetes...
A-u-comparing = Comparando versiones de los paquetes...
A-u-no-upgrades = No es necesario actualizar paquetes de AUR.
A-u-to-upgrade = Paquetes de AUR a actualizar:
...
# Snapshots (-B)
B-saved = Estado del paquete salvado.
...
C-b-target = Haciendo una copia de seguridad de la caché en { $target }
...
C-c-keep = Se mantendrán { $pkgs } ficheros de cada paquete. El resto se eliminarán.
C-c-freed = { $bytes } megabytes liberados.

C-downgrade-which = ¿Qué versión de { $pkg } deseas?
...
# Logs (-L)
L-first = Primera instalación
L-upgrades = Actualizaciones
L-recent = Acciones Recientes
...
env-pconf = No fue posible analizar su archivo pacman.conf.
