...
# AUR Packages (-A)
A-install-deps = Determinando as dependências...
A-install-repo-pkgs = Dependências no repositório:
A-install-aur-pkgs = AUR Pacotes:
# A-install-path-comp = Failed to extract final component of: { $path }
A-install-ignored = { $file } está marcado como Ignorado. Deseja instalar mesmo assim?
...
A-build-fail = Falha na compilação.
# A-build-e-pkgctl = Building within an isolated chroot failed.
A-build-e-makepkg = Ocorreu um erro ao executar makepkg.
...
A-build-continue = Gostaria de continuar mesmo assim?

A-i-repo = Repositório
A-i-version = Versão
A-i-status = Estado no AUR
A-i-maintainer = Mantenedor
A-i-proj-url = URL do projeto
A-i-aur-url = URL no AUR
A-i-license = Licença
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Dependências
A-i-make = Dependências de compilação
# A-i-opt = Optional Deps
# A-i-check = Check Deps
A-i-votes = Votos
A-i-pop = Popularidade
A-i-desc = Descrição
# A-i-keywords = Keywords
# A-i-submitted = Submitted
# A-i-updated = Updated

A-u-fetch-info = Obtendo informação dos pacotes...
A-u-comparing = Comparando versões dos pacotes...
A-u-no-upgrades = Nenhum pacote do AUR precisa de atualização.
A-u-to-upgrade = Pacotes do AUR para atualizar:
...
# Snapshots (-B)
B-saved = Estado de pacote salvo.
...
C-b-target = Backup do cache sendo feito em { $target }
...
C-c-keep = { $pkgs } arquivos de cada pacote serão mantidos. O resto será removido.
# C-c-freed = { $bytes } freed.

C-downgrade-which = Qual versão de { $pkg } deseja?"
...
# Logs (-L)
L-first = Primeira instalação
L-upgrades = Atualizações
L-recent = Ações Recentes
...
env-pconf = Não foi possível interpretar o arquivo pacman.conf.
