...
# AUR Packages (-A)
A-install-deps = Určování závislostí...
A-install-repo-pkgs = Závislosti úložiště:
A-install-aur-pkgs = Balíčky AUR:
# A-install-path-comp = Failed to extract final component of: { $path }
A-install-ignored = { $file } je označeno jako ignorováno. Přesto nainstalovat?

# A-build-prep = Preparing build directories...
A-build-pkg = Kompilace { $pkg }...
# A-build-diff = Display diffs of build files?
# A-build-hotedit-pkgbuild = Edit the PKGBUILD?
A-build-hotedit-install = Chcete upravit soubor .install?
A-build-fail = Budování se nezdařilo.
# A-build-e-pkgctl = Building within an isolated chroot failed.
A-build-e-makepkg = Došlo k chybě makepkg.
...
A-build-pull = Nepodařilo se 'git stáhnout' nejnovější aktualizace.
A-build-continue = Chcete přesto pokračovat?

A-i-repo = Úložiště
A-i-version = Verze
A-i-status = Stav AUR
A-i-maintainer = Udržovatel
A-i-proj-url = URL của Dự án
A-i-aur-url = URL pro AUR
A-i-license = License
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Závisí na
A-i-make = Závislosti kompilace
...
A-u-fetch-info = Načítání informací o balíčku...
A-u-comparing = Porovnání verzí balíčků...
A-u-no-upgrades = Není nutné žádné aktualizace balíčku AUR.
A-u-to-upgrade = Balíčky AUR k aktualizaci:
...
# Snapshots (-B)
B-saved = Stav balíčku byl uložen.
...
C-b-target = Zálohování mezipaměti do { $target }
...
C-c-keep = { $pkgs } každého souboru balíčku bude zachován. Zbytek bude smazán.
C-c-freed = Uvolněno { $bytes }.

C-downgrade-which = Jakou verzi { $pkg } chcete?
...
# Logs (-L)
L-first = První instalace
L-upgrades = Aktualizace
L-recent = Nedávné akce
