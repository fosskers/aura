...
# AUR Packages (-A)
A-install-deps = Bezig met vaststellen van afhankelijkheden…
A-install-repo-pkgs = Pakketbron-afhankelijkheden:
# A-install-aur-pkgs = AUR packages:
# A-install-path-comp = Failed to extract final component of: { $path }
A-install-ignored = { $file } is gemarkeerd als genegeerd. Wilt u het pakket toch installeren?

# A-build-prep = Preparing build directories...
A-build-pkg = Bezig met bouwen van { $pkg }…
...
A-build-fail = Het bouwen is mislukt.
# A-build-e-pkgctl = Building within an isolated chroot failed.
A-build-e-makepkg = Er is een fout opgetreden in makepkg.
...
A-build-continue = Wilt u toch doorgaan?

A-i-repo = Pakketbron
A-i-version = Versie
A-i-status = AUR-status
A-i-maintainer = Eigenaar
A-i-proj-url = Project-url
A-i-aur-url = AUR-url
A-i-license = Licentie
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Afhankelijk van
A-i-make = Bouwafhankelijkheden
# A-i-opt = Optional Deps
# A-i-check = Check Deps
A-i-votes = Aantal stemmen
A-i-pop = Populariteit
A-i-desc = Beschrijving
# A-i-keywords = Keywords
# A-i-submitted = Submitted
# A-i-updated = Updated

A-u-fetch-info = Bezig met ophalen van pakketinformatie…
A-u-comparing = Bezig met vergelijken van pakketversies…
A-u-no-upgrades = Er hoeven geen AUR-pakketten te worden bijgewerkt.
# A-u-no-upgrades = No AUR package upgrades necessary.
A-u-to-upgrade = Bij te werken AUR-pakketten:
...
C-b-target = Bezig met back-uppen van cache aan naar { $target }
...
C-c-keep = { $pkgs } van elk pakketbestand wordt bewaard. De rest wordt gewist.
C-c-freed = { $bytes } vrijgemaakt.

C-downgrade-which = Welke versie van { $pkg } wilt u?
...
# Logs (-L)
L-first = Eerste installatie
L-upgrades = Opwaarderingen
L-recent = Onlangs uitgevoerde acties
...
env-pconf = Uw pacman.conf-bestand kan niet worden verwerkt.
...
# Misc.
# proceed = Proceed?
proceed-affirmative = j
proceed-affirmative-alt = J
proceed-negative = n
