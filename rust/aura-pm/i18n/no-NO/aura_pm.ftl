...
# AUR Packages (-A)
A-install-deps = Bestemmer avhengigheter...
A-install-repo-pkgs = Avhengigheter fra depotet:
A-install-aur-pkgs = AUR Pakker:
...
A-i-maintainer = Vedlikeholder
A-i-proj-url = Prosjekt-URL
A-i-aur-url = AUR URL
A-i-license = Lisens
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Er avhengig av
A-i-make = Byggavhengigheter
# A-i-opt = Optional Deps
# A-i-check = Check Deps
A-i-votes = Stemmer
A-i-pop = Popularitet
A-i-desc = Beskrivelse
# A-i-keywords = Keywords
# A-i-submitted = Submitted
# A-i-updated = Updated

A-u-fetch-info = Henter pakkeinformasjon...
A-u-comparing = Sammenligner pakkeversjoner...
A-u-no-upgrades = Ingen pakkeoppgradering fra AUR nødvendig.
A-u-to-upgrade = AUR-pakker å oppgradere:
...
C-b-target = Tar backup på cache til { $target }
...
C-c-keep = { $pkgs } av hver pakkefil blir beholdt. Resten vil bli slettet.
# C-c-freed = { $bytes } freed.

C-downgrade-which = Hvilken versjon av { $pkg } vil du ha?
...
# Logs (-L)
L-first = Første installasjon
L-upgrades = Oppgraderinger
L-recent = Nylige hendelser
