# TODO What's the name for Vietnamese in Vietnamese?
language-name = Vietnamese

# AUR Packages (-A)
A-install-deps = Xác định các gói phụ thuộc...
A-install-repo-pkgs = Các repo phụ thuộc:
A-install-aur-pkgs = Gói AUR:
# A-install-path-comp = Failed to extract final component of: { $path }
A-install-ignored = { $file } được đánh dấu là Bỏ qua. Vẫn cài đặt nó?

# A-build-prep = Preparing build directories...
A-build-pkg = Đang build { $pkg }...
# A-build-diff = Display diffs of build files?
# A-build-hotedit-pkgbuild = Edit the PKGBUILD?
A-build-hotedit-install = Bạn có muốn chỉnh sửa tệp .install?
A-build-fail = Build thất bại.
# A-build-e-pkgctl = Building within an isolated chroot failed.
A-build-e-makepkg = Có lỗi khi makepkg.
...
A-build-pull = Thất bại trong việc 'git pull' để cập nhật.
A-build-continue = Bạn có muốn tiếp tục không?

# A-i-repo = Repository
A-i-version = Phiên bản
A-i-status = Trạng thái AUR
# A-i-maintainer = Maintainer
A-i-proj-url = URL của Dự án
A-i-aur-url = URL của AUR
A-i-license = Bản quyền
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Phụ thuộc vào
# A-i-make = Make Deps
...
A-u-fetch-info = Cập nhật thông tin của gói...
A-u-comparing = So sánh phiên bản của gói...
A-u-no-upgrades = Không có cập nhật cho các gói AUR.
A-u-to-upgrade = Cập nhật các gói AUR:
...
C-b-target = Sao lưu cache vào { $target }
...
C-c-keep = Sẽ giữ lại { $pkgs } tệp của gói. Xóa bỏ phần còn lại.
C-c-freed = Giải phóng { $bytes }.

C-downgrade-which = Bạn muốn sử dụng phiên bản nào của { $pkg }?
...
# Logs (-L)
L-first = Cài đặt lần đầu
L-upgrades = Cập nhật
L-recent = Hoạt động gần nhất
...
env-pconf = Không thể lấy dữ liệu từ tệp pacman.conf của bạn.
