...
# AUR Packages (-A)
A-install-deps = Détermination des dépendances en cours…
A-install-repo-pkgs = Dépendances du dépôt:
A-install-aur-pkgs = AUR Paquets:
...
A-build-pkg = Compilation de { $pkg }...
...
A-build-fail = Compilation échouée.
...
A-build-continue = Voulez-vous tout de même continuer?

A-i-repo = Dépôt
A-i-version = Version
A-i-status = Statut de AUR
A-i-maintainer = Mainteneur
A-i-proj-url = URL du projet
A-i-aur-url = URL AUR
A-i-license = Licence
# A-i-group = Groups
# A-i-provides = Provides
A-i-depends = Dépends de
A-i-make = Dépendances de compilation
...
A-u-fetch-info = Obtention des informations des paquets en cours…
A-u-comparing = Comparaison des versions des paquets en cours…
A-u-no-upgrades = Aucune mise à jour de paquet AUR n'est nécessaire.
A-u-to-upgrade = Paquets AUR à mettre à jour:
...
# Snapshots (-B)
B-saved = État des paquets sauvegardé.
...
C-b-target = Copie de sauvegarde dans { $target }.
...
C-c-keep = { $pkgs } fichiers de chaque paquet sera conservé. Le reste sera supprimé.
# C-c-freed = { $bytes } freed.

# getDowngradeChoice_1
C-downgrade-which = Quelle version de { $pkg } voulez-vous?
...
# Logs (-L)
# firstInstall
L-first = Première installation
L-upgrades = Mises à jours
L-recent = Actions récentes
...
# Misc.
# proceed = Proceed?
proceed-affirmative = o
proceed-affirmative-alt = O
proceed-negative = n
